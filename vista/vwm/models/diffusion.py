...
class DiffusionEngine(LightningModule):
    ...
    # def decode_first_stage_grad(self, z, overlap=3):
    #     z = z / self.scale_factor
    #     n_samples = default(self.en_and_decode_n_samples_a_time, z.shape[0])
    #     all_out = []
    #     with torch.autocast("cuda", enabled=not self.disable_first_stage_autocast):
    ...
    #                 context_z = torch.cat((previous_z, current_z), dim=0)
    #                 previous_z = current_z[-overlap:]
    #                 out = self.first_stage_model.decode(context_z, **kwargs)  # [L, C, H, W]

    #                 if not all_out:
    #                     # first block, just append
    #                     all_out.append(out)
    #                 else:
    #                     # last block from all_out
    #                     prev = all_out[-1]             # [L_prev, C, H, W]
    #                     # non-overlapping prefix from prev
    #                     prev_prefix = prev[:-overlap]  # [L_prev - overlap, C, H, W]
    #                     # blended overlap region
    #                     blended_overlap = (prev[-overlap:] + out[:overlap]) / 2.0
    #                     # remaining new part
    #                     new_tail = out[overlap:]

    #                     # build a NEW tensor, no in-place modification
    #                     merged = torch.cat(
    #                         [prev_prefix, blended_overlap, new_tail],
    #                         dim=0,
    #                     )
    #                     all_out[-1] = merged
    #         else:
    ...
    def decode_first_stage_grad(self, z, overlap=3):
        ...
        # Wrapper for checkpointing (must take tensors as input)
        def run_decode(z_in, **kwargs):
            return self.first_stage_model.decode(z_in, **kwargs)
        ...
        with torch.autocast("cuda", enabled=not self.disable_first_stage_autocast):
            if overlap < n_samples:
                ...
                for current_z in z[overlap:].split(n_samples - overlap, dim=0):
                    ...
                    out = checkpoint(run_decode, context_z, use_reentrant=False, **kwargs)
                    ...
                    if not all_out:
                        ...
                        # 1. Non-overlapping prefix
                        prev_prefix = prev[:-overlap]       # [L_prev - overlap, ...]
                        
                        # 2. Blended overlap region
                        blended_overlap = (prev[-overlap:] + out[:overlap]) / 2.0
                        
                        # 3. New tail
                        new_tail = out[overlap:]

                        # 4. Concatenate new tensor (Safe!)
                        # Note: We replace the last element. The previous 'prev' tensor 
                        # is now disconnected from the list, but its graph is preserved 
                        # in 'merged' via the prev_prefix and blended_overlap parts.
                        merged = torch.cat(
                            [prev_prefix, blended_overlap, new_tail],
                            dim=0,
                        )
                ...
                for current_z in z.split(n_samples, dim=0):
                    ...
                    # USE CHECKPOINTING HERE
                    out = checkpoint(run_decode, current_z, use_reentrant=False, **kwargs)
                    all_out.append(out)
