...
class wm_args:
    ########################### training args ##############################
    # Model paths — resolved relative to ctrl_world/ckpts/ (symlinks).
    svd_model_path = os.path.join(_CFG_DIR, "ckpts", "stable-video-diffusion-img2vid")
    clip_model_path = os.path.join(_CFG_DIR, "ckpts", "clip-vit-base-patch32")
    ckpt_path = os.path.join(_CFG_DIR, "ckpts", "Ctrl-World", "coffee_bag.pt")
    pi_ckpt = None  # robotic policy weights are not used in this self-contained example
    ...
    # meta info
    dataset_meta_info_path = os.path.join(_CFG_DIR, "dataset_meta_info")
    ...
    # logs parameters
    debug = False
    tag = 'candy_coffee_history_lr1e-6' #'pickup_history_lr1e-6'#'jenga_history_lr1e-6'#'jenga_nohistory_lr1e-6'#'jenga_train'
    # tag = 'pickup_train'
    output_dir = f"model_ckpt/{tag}"
    wandb_run_name = tag
    wandb_project_name = "ctrl-world-finetune"


    # training parameters
    learning_rate= 1e-6 #1e-5 # 5e-6
    ...
    max_grad_norm = 1.0
    # LoRA (set use_lora=True to freeze UNet and train LoRA adapters only)
    use_lora = False
    lora_rank = 16
    lora_alpha = 16
    lora_dropout = 0.05
    lora_target = 'attention'  # 'attention' (to_q/k/v/out only) or 'all-linear' (all nn.Linear)
    # for val
    video_num= 10
    ...
    action_adapter = os.path.join(_CFG_DIR, "models", "action_adapter", "model2_15_9.pth")
    ...
    # wm
    data_stat_path = os.path.join(_CFG_DIR, "dataset_meta_info", "droid", "stat.json")
